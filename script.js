// ============================================================
//  script.js – ПОЛНЫЙ ФАЙЛ (БЕЗ СОКРАЩЕНИЙ)
// ============================================================
...
function getTempleName(id) {
    var t = data.temples.find(function(temple) { return temple.id === id; });
    return t ? t.name : '?';
}

function getTempleNames(ids) {
    if (!ids || !ids.length) return 'не привязан';
    return ids.map(function(id) { return getTempleName(id); }).join(', ');
}

function getTemplePhoto(temple) {
    return temple?.photo?.trim() || 'placeholder.jpg';
}
...
async function hashPassword(password) {
    ...
    var dataBytes = encoder.encode(password);
    var hashBuffer = await crypto.subtle.digest('SHA-256', dataBytes);
...
var allPermissions = ['manage_temples', 'manage_clergy', 'manage_schedule', 'manage_news', 'manage_announcements', 'manage_sunday_schools', 'manage_about', 'manage_worship', 'manage_users', 'manage_ai', 'manage_opechenie', 'view_logs'];
...
function addLog(action, details) {
    ...
    db.ref('logs').push(logEntry).catch(function(err) { console.error('Ошибка сохранения лога:', err); });
...
function loadData() {
    ...
    dataLoaded = true;
    var stored = localStorage.getItem('blago_data');
    if (stored) {
        try {
    ...
    logsListener.on('value', function(snapshot) {
        var logsData = snapshot.val();
        if (logsData) {
            var logsArray = Object.values(logsData).sort(function(a, b) {
                return new Date(b.timestamp) - new Date(a.timestamp);
            });
            data.logs = logsArray;
            saveToLocalStorage();
            var adminContent = document.getElementById('adminSectionContent');
            if (adminContent && adminContent.closest('.admin-modal') && adminContent.closest('.admin-modal').classList.contains('visible')) {
                var section = adminContent.dataset.currentSection;
...
function migrateData() {
    var arrayKeys = ['temples', 'clergy', 'schedules', 'news', 'announcements', 'sundaySchools', 'teachers', 'faq', 'users', 'opechenie', 'logs'];
    ...
    if (!data.users || data.users.length === 0) {
        hashPassword('Makar27.05.2014').then(function(hash) {
    ...
    if (data.teachers && data.teachers.length) {
        var bogatkoIdx = data.teachers.findIndex(function(t) { return t.id === 3; });
        var levshevichIdx = data.teachers.findIndex(function(t) { return t.id === 2; });
        if (bogatkoIdx !== -1 && levshevichIdx !== -1 && bogatkoIdx > levshevichIdx) {
            var temp = data.teachers[bogatkoIdx];
            data.teachers[bogatkoIdx] = data.teachers[levshevichIdx];
            data.teachers[levshevichIdx] = temp;
        }
    }
    setDefaultPhotos();
}
...
function saveToLocalStorage() {
    localStorage.setItem('blago_data', JSON.stringify({ data: data, nextId: nextId }));
    localStorage.setItem('vision_mode', visionMode ? 'on' : 'off');
}

function saveToFirebase() {
    db.ref('data').set({ data: data, nextId: nextId }).then(function() {
...
function setDefaultPhotos() {
    data.temples.forEach(function(t) { if (!t.photo) t.photo = 'placeholder.jpg'; });
    data.clergy.forEach(function(c) { if (!c.photo) c.photo = 'placeholder.jpg'; });
    data.sundaySchools.forEach(function(s) { if (!s.photo) s.photo = 'placeholder.jpg'; });
    data.teachers.forEach(function(t) { if (!t.photo) t.photo = 'placeholder.jpg'; });
}
...
function rebuildNav() {
    ...
    var oldNav = topBar.querySelector('nav');
    if (oldNav) oldNav.remove();
    var oldModal = document.getElementById('menuModal');
    if (oldModal) oldModal.remove();

    var tools = topBar.querySelector('.tools');
    if (!tools) return;
    ...
    modal.id = 'menuModal';
    modal.style.cssText = 'position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.6); backdrop-filter:blur(4px); z-index:200; display:none; align-items:center; justify-content:center; visibility:hidden; opacity:0; transition:opacity 0.3s ease, visibility 0.3s ease; pointer-events:none;';
    var modalContent = document.createElement('div');
    modalContent.className = 'menu-modal-content';
    modalContent.style.cssText = 'background:var(--card-bg, #ffffff); width:95%; max-width:400px; max-height:90vh; overflow-y:auto; border-radius:32px; padding:1.5rem 1.5rem 2rem; position:relative; box-shadow:0 20px 60px rgba(0,0,0,0.3); transform:scale(0.9); transition:transform 0.3s ease; pointer-events:auto;';
    var closeBtn = document.createElement('button');
    closeBtn.className = 'menu-modal-close';
    closeBtn.innerHTML = '✕';
    closeBtn.style.cssText = 'position:sticky; top:0; float:right; font-size:2rem; background:none; border:none; cursor:pointer; color:#333; z-index:10; padding:0 0.5rem; pointer-events:auto;';
    closeBtn.setAttribute('aria-label', 'Закрыть меню');
    modalContent.appendChild(closeBtn);

    var title = document.createElement('h3');
    title.textContent = 'Меню';
    title.style.cssText = 'font-family:"Cormorant Uncial", Georgia, serif; color:var(--primary, #1a2f40); text-align:center; margin-bottom:1rem; font-size:1.6rem; border-bottom:2px solid var(--gold, #c9aa5f); padding-bottom:0.5rem;';
    modalContent.appendChild(title);

    var linksList = document.createElement('div');
    linksList.style.cssText = 'display:flex; flex-direction:column; gap:0.8rem; margin-top:0.5rem;';
    links.forEach(function(l) {
        var a = document.createElement('a');
        a.href = l.page === 'main' ? 'index.html' : l.page + '.html';
        a.dataset.page = l.page;
        a.textContent = l.text;
        a.style.cssText = 'display:block; padding:0.7rem 1rem; background:var(--bg, #f9f6ef); border-radius:16px; text-align:center; font-size:1.1rem; font-family:var(--font, Georgia, serif); color:var(--text, #2c2a24); text-decoration:none; border:1px solid var(--border, #e8ddd0); transition:background 0.2s, transform 0.2s; pointer-events:auto;';
        a.addEventListener('mouseenter', function() {
            this.style.background = 'var(--gold, #c9aa5f)';
            this.style.color = 'white';
        });
        a.addEventListener('mouseleave', function() {
            this.style.background = 'var(--bg, #f9f6ef)';
            this.style.color = 'var(--text, #2c2a24)';
        });
    ...
    var closeBtnEl = modalEl.querySelector('.menu-modal-close');
    ...
    modalEl.querySelectorAll('a[data-page]').forEach(function(a) {
        a.addEventListener('click', function(e) {
            closeModal(e);
        });
    });

    var currentPage = document.body.dataset.page || 'main';
...
function renderCurrentPage() {
    ...
    var page = document.body.dataset.page || 'main';
...
function renderMainPage() {
    var container = document.getElementById('mainContent');
    if (!container) return;
    var html = '<div class="hero-banner" onclick="window.location.href=\'temple-1.html\'">' +
    ...
    var news = data.news.slice().sort(function(a, b) { return new Date(b.date) - new Date(a.date); }).slice(0, 3);
    ...
    var ann = data.announcements.slice().sort(function(a, b) { return new Date(b.date) - new Date(a.date); }).slice(0, 3);
    if (!ann.length) html += '<p>' + t('no-announcements') + '</p>';
    else {
        ann.forEach(function(a) {
            html += '<div><p>' + escapeHtml(a.text) + '</p><span style="font-size:0.85rem;color:#999;">' + escapeHtml(a.date) + '</span></div>';
        });
...
function initInfiniteCarousel() {
    ...
    var templeItems = data.temples.filter(function(t) { return t.id !== 1; }).map(function(t) {
        var imgSrc = getTemplePhoto(t);
    ...
    var totalItems = templeItems.length;
    if (totalItems === 0) { track.innerHTML = '<p>Нет храмов для отображения</p>'; return; }

    var cloneFirst = templeItems.slice(0, 2).join('');
    var cloneLast = templeItems.slice(-2).join('');
    var allItems = cloneLast + templeItems.join('') + cloneFirst;
    ...
    function updateCarousel(animate) {
        ...
        var gap = getGap();
        var offset = currentIndex * (width + gap);
    ...
    function goTo(index, animate) {
        ...
        setTimeout(function() {
            ...
            if (currentIndex >= totalItems + 2) { currentIndex = 2;
                updateCarousel(false); } else if (currentIndex < 2) { currentIndex = totalItems + 1;
    ...
    track.addEventListener('mouseleave', function() { autoplay = setInterval(next, 4000); });

    var resizeTimeout;
    window.addEventListener('resize', function() {
        clearTimeout(resizeTimeout);
        resizeTimeout = setTimeout(function() { updateCarousel(false); }, 100);
    });
...
function renderTemplesList(container) {
    var html = '<h2>' + t('temples-title') + '</h2><div class="grid">';
    ...
    html += '</div>';
    container.innerHTML = html;
    container.querySelectorAll('.grid-item[data-type="temple"]').forEach(function(el) {
        el.addEventListener('click', function() { window.location.href = 'temple-' + this.dataset.id + '.html'; });
    });
}

// ---------- МОДАЛЬНОЕ ОКНО ДЛЯ ХРАМА ----------
function openTempleModal(tab, templeId) {
    var modal = document.getElementById('templeModal');
    if (!modal) {
    ...
    var body = document.getElementById('templeModalBody');
    var temple = data.temples.find(function(t) { return t.id === templeId; });
    ...
    switch (tab) {
        case 'schedule':
            ...
            var schedules = data.schedules.filter(function(s) { return s.templeId === templeId; });
            if (schedules.length) {
        ...
        case 'contacts':
            ...
            var address = temple.address ? temple.address.replace(/ул\. /g, 'ул.\u00A0').replace(/г\. /g, 'г.\u00A0').replace(/д\. /g, 'д.\u00A0').replace(/п\. /g, 'п.\u00A0').replace(/аг\. /g, 'аг.\u00A0') : '';
        ...
        case 'clergy':
            ...
            var clergy = data.clergy.filter(function(c) { return c.templeIds && c.templeIds.includes(templeId); });
            if (clergy.length) {
                content += '<div class="clergy-list">';
                clergy.forEach(function(c) {
        ...
        case 'schools':
            ...
            var schools = data.sundaySchools.filter(function(s) { return s.templeId === templeId; });
            if (schools.length) {
    ...
    body.querySelectorAll('.clergy-card').forEach(function(el) {
        el.addEventListener('click', function() {
...
function renderTempleDetail(container, id) {
    if (!data.temples || data.temples.length === 0) { setTimeout(function() { renderTempleDetail(container, id); }, 300); return; }
    var temple = data.temples.find(function(t) { return t.id === id; });
    if (!temple) { container.innerHTML = '<p>Храм не найден</p>'; return; }
    var photoSrc = getTemplePhoto(temple);
    var phoneNumber = temple.phone || '+375291234567';
    var address = temple.address ? temple.address.replace(/ул\. /g, 'ул.\u00A0').replace(/г\. /g, 'г.\u00A0').replace(/д\. /g, 'д.\u00A0').replace(/п\. /g, 'п.\u00A0').replace(/аг\. /g, 'аг.\u00A0') : '';
    var html = '<div class="detail-back" onclick="history.back()">' + t('back') + '</div>' +
        ...
        '</div>';
    container.innerHTML = html;
    container.querySelectorAll('.temple-action-btn[data-tab]').forEach(function(btn) {
        btn.addEventListener('click', function() { openTempleModal(this.dataset.tab, id); });
    });
    var historyBtns = container.querySelectorAll('.history-btn');
    var historyTabs = {
    ...
    historyBtns.forEach(function(btn) {
        btn.addEventListener('click', function() {
            ...
            historyBtns.forEach(function(b) { b.classList.remove('active'); });
            this.classList.add('active');
            Object.keys(historyTabs).forEach(function(key) {
...
function renderClergyList(container) {
    var html = '<h2>' + t('clergy-title') + '</h2><div class="grid" id="clergyList">';
    data.clergy.forEach(function(c) {
    ...
    html += '</div>';
    container.innerHTML = html;
    container.querySelectorAll('.grid-item[data-type="clergy"]').forEach(function(el) {
        el.addEventListener('click', function() { window.location.href = 'clergy-detail.html?id=' + this.dataset.id; });
    });
}

function renderClergyDetail(id) {
    if (!data.clergy || data.clergy.length === 0) { setTimeout(function() { renderClergyDetail(id); }, 300); return; }
    var c = data.clergy.find(function(cl) { return cl.id === id; });
...
function getScheduleHTML() {
    var html = '<h2>' + t('schedule-title') + '</h2><div class="card"><h3>' + t('select-temple') + '</h3><select id="scheduleTempleSelect" style="width:100%; padding:0.6rem; border-radius:16px; border:1px solid var(--border);"><option value="">' + t('choose-temple') + '</option>';
    data.temples.forEach(function(t) { html += '<option value="' + t.id + '">' + escapeHtml(t.name) + '</option>'; });
    html += '</select></div><div id="scheduleDisplay"></div>';
    return html;
}

function initScheduleSelect(container) {
    var select = container.querySelector('#scheduleTempleSelect');
    if (!select) return;
    select.addEventListener('change', function() {
        var tid = parseInt(this.value);
        var display = container.querySelector('#scheduleDisplay');
        if (!tid) { display.innerHTML = '<p>' + t('select-temple') + '</p>'; return; }
        var schedules = data.schedules.filter(function(s) { return s.templeId === tid; });
        if (!schedules.length) { display.innerHTML = '<p>' + t('no-schedule') + '</p>'; return; }
        var table = '<table class="schedule-table"><thead><tr><th>' + t('date') + '</th><th>' + t('event') + '</th></tr></thead><tbody>';
        schedules.forEach(function(s) { table += '<tr><td>' + escapeHtml(s.date) + '</td><td>' + escapeHtml(s.event) + '</td></tr>'; });
        table += '</tbody></table>';
...
function renderNewsList(container) {
    var html = '<h2>' + t('news-title') + '</h2><div style="margin-bottom: 1rem;"><a href="https://t.me/dzrzh_blag" target="_blank" class="btn-telegram" style="background: #0088cc; color: white; padding: 0.5rem 1.2rem; border-radius: 40px; text-decoration: none; display: inline-block; font-weight: bold;">📢 Подписаться на Telegram-канал</a></div>';
    var news = data.news || [];
    if (!news.length) html += '<p>' + t('no-news') + '</p>';
    else {
...
function renderAnnouncementsList(container) {
    var html = '<h2>' + t('announcements-title') + '</h2>';
    var ann = data.announcements || [];
    if (!ann.length) html += '<p>' + t('no-announcements') + '</p>';
    else {
        var sorted = ann.slice().sort(function(a, b) { return new Date(b.date) - new Date(a.date); });
        sorted.forEach(function(a) {
            html += '<div class="card"><p>' + escapeHtml(a.text) + '</p><p style="font-size:0.85rem;color:#999;">' + escapeHtml(a.date) + '</p></div>';
        });
    }
    container.innerHTML = html;
}
...
function renderSundaySchoolsList(container) {
    var html = '<h2>' + t('sunday-school-title') + '</h2>' +
        ...
        '</ul></div>' +
        '<div class="grid">';
    (data.sundaySchools || []).forEach(function(s) {
        var temple = data.temples.find(function(t) { return t.id === s.templeId; });
    ...
    html += '</div>';
    container.innerHTML = html;
    container.querySelectorAll('.grid-item[data-type="sunday-school"]').forEach(function(el) {
        el.addEventListener('click', function() { window.location.href = 'sunday-school-detail.html?id=' + this.dataset.id; });
    });
}

function renderSundaySchoolDetail(id) {
    if (!data.sundaySchools || data.sundaySchools.length === 0) { setTimeout(function() { renderSundaySchoolDetail(id); }, 300); return; }
    var school = data.sundaySchools.find(function(s) { return s.id === id; });
    if (!school) { document.getElementById('mainContent').innerHTML = '<p>Школа не найдена</p>'; return; }
    var container = document.getElementById('mainContent');
    var temple = data.temples.find(function(t) { return t.id === school.templeId; });
    var teachers = data.teachers.filter(function(t) { return t.schoolId === id; });
    var roleOrder = { 'Директор': 1, 'Преподаватель': 2, 'Воспитатель': 3 };
    teachers.sort(function(a, b) { return (roleOrder[a.role] || 99) - (roleOrder[b.role] || 99); });

    var teachersHtml = '';
    if (teachers.length) {
        teachersHtml = '<h3 style="margin-top: 1.5rem; margin-bottom: 1rem;">👨‍🏫 Преподаватели и сотрудники</h3>' +
            '<div class="clergy-list" style="display: flex; flex-wrap: wrap; gap: 1.5rem; justify-content: center;">';
        teachers.forEach(function(t) {
            teachersHtml += '<div class="clergy-card" style="width: 220px; text-align: center; background: var(--card-bg); padding: 1rem; border-radius: 16px; border: 1px solid var(--border);">' +
...
function renderWorshipPage(container) {
    ...
    var storageKey = 'worshipTab';
    var activeTab = getStoredTab(storageKey, 'schedule');

    var html = '<h2>' + t('nav-worship') + '</h2>' +
        '<div class="card">' +
    ...
    var prayers = data.worship?.prayers || [];
    if (!prayers.length) html += '<p>Молитвы не добавлены.</p>';
    else prayers.forEach(function(p) { html += '<div class="prayer-item"><strong>' + escapeHtml(p.title) + '</strong><p>' + escapeHtml(p.text) + '</p></div>'; });
    html += '</div>';
    ...
    var interpretations = data.worship?.interpretations || [];
    if (!interpretations.length) html += '<p>Толкования не добавлены.</p>';
    else interpretations.forEach(function(i) { html += '<div class="interpretation-item"><strong>' + escapeHtml(i.title) + '</strong><p>' + escapeHtml(i.text) + '</p></div>'; });
    html += '</div>';
    html += '<div class="worship-block ' + (activeTab === 'sacraments' ? 'active' : '') + '" id="worship-sacraments">';
    var sacraments = data.worship?.sacraments || [];
    if (!sacraments.length) html += '<p>Подготовка к таинствам не добавлена.</p>';
    else sacraments.forEach(function(s) { html += '<div class="sacrament-item"><strong>' + escapeHtml(s.title) + '</strong><p>' + escapeHtml(s.text) + '</p></div>'; });
    html += '</div></div></div>';
    container.innerHTML = html;
    ...
    function loadSaintsWidget() {
        var containerEl = document.getElementById('saintsContainer');
        if (!containerEl) return;
        containerEl.innerHTML = '';
        var widgetDiv = document.createElement('div');
        widgetDiv.className = 'azbyka-saints';
        containerEl.appendChild(widgetDiv);
        var oldScript = document.querySelector('script[src="https://azbyka.ru/days/js/api.min.js"]');
    ...
    container.querySelectorAll('.worship-tab-btn').forEach(function(btn) {
        btn.addEventListener('click', function() {
            var tabId = this.dataset.tab;
            setStoredTab(storageKey, tabId);
            activeTab = tabId;
            container.querySelectorAll('.worship-tab-btn').forEach(function(b) {
            ...
            this.style.color = 'white';
            container.querySelectorAll('.worship-block').forEach(function(block) { block.classList.remove('active'); });
            var target = document.getElementById('worship-' + tabId);
    ...
    var activeBtn = container.querySelector('.worship-tab-btn[data-tab="' + activeTab + '"]');
    if (activeBtn) { activeBtn.classList.add('active');
        activeBtn.style.background = 'var(--gold)';
        activeBtn.style.color = 'white'; } else if (container.querySelector('.worship-tab-btn')) {
        var firstBtn = container.querySelector('.worship-tab-btn');
        firstBtn.classList.add('active');
        firstBtn.style.background = 'var(--gold)';
        firstBtn.style.color = 'white';
        var firstTab = firstBtn.dataset.tab;
        setStoredTab(storageKey, firstTab);
        var firstBlock = document.getElementById('worship-' + firstTab);
...
function renderFaqPage(container) {
    ...
    if (!faq.length) html += '<p>' + t('no-faq') + '</p>';
        ...
        var sorted = faq.slice().sort(function(a, b) { return new Date(b.date) - new Date(a.date); });
    ...
    if (hasPermission(currentUser, 'manage_ai')) {
        html += '<div class="card" id="aiBlock">' +
            '<h2>' + t('ai-chat') + '</h2>' +
    ...
    if (feedbackForm) {
        feedbackForm.addEventListener('submit', async function(e) {
            ...
            try {
                var response = await fetch('send.php', { method: 'POST', body: formData });
...
async function askAI() {
    ...
    var contentDiv = document.getElementById('aiResponseContent');
    if (!answerDiv || !contentDiv) return;
    answerDiv.style.display = 'block';
    contentDiv.textContent = '⏳ ИИ думает...';
    setTimeout(function() { contentDiv.textContent = '🤖 ИИ временно недоступен. Пожалуйста, попробуйте позже.'; }, 1000);
}
async function adminAskAI() {
    var questionInput = document.getElementById('adminAIQuestion');
    if (!questionInput) return;
    var question = questionInput.value.trim();
    if (!question) { alert('Введите вопрос'); return; }
    var answerDiv = document.getElementById('adminAIAnswer');
    var contentDiv = document.getElementById('adminAIResponseContent');
    if (!answerDiv || !contentDiv) return;
    answerDiv.style.display = 'block';
    contentDiv.textContent = '⏳ ИИ думает...';
    setTimeout(function() { contentDiv.textContent = '🤖 ИИ временно недоступен. Пожалуйста, попробуйте позже.'; }, 1000);
...
function renderAdminAI(container) {
    container.innerHTML = '<h3>🤖 ИИ-помощник (заглушка)</h3>' +
        '<div class="card">' +
...
function renderOpecheniePage(container) {
    var html = '<h2>Окормление</h2>';
    if (!data.opechenie || data.opechenie.length === 0) {
        html += '<p>Нет данных об окормлении.</p>';
    } else {
        html += '<div class="grid">';
        data.opechenie.forEach(function(item) {
            var templeName = getTempleName(item.templeId);
...
// ========== АДМИН-ПАНЕЛЬ ==========
var adminModal = null, adminModalContent = null;
...
function openAdminModal() {
    ...
    if (!currentUser) renderAdminLogin();
...
function renderAdminLogin() {
    ...
    document.getElementById('doLogin').addEventListener('click', async function() {
        ...
        var user = data.users.find(function(u) { return u.username === login; });
        if (!user) { alert(t('wrong-password')); return; }
        var storedPassword = user.password || '';
        if (isHash(storedPassword)) {
            var inputHash = await hashPassword(pass);
            if (inputHash === storedPassword) { currentUser = user;
                renderAdminDashboard(); } else alert(t('wrong-password'));
        } else {
...
function renderAdminDashboard() {
    var hasUsersPerm = hasPermission(currentUser, 'manage_users');
    var hasLogsPerm = hasPermission(currentUser, 'view_logs');
    var hasTemplesPerm = hasPermission(currentUser, 'manage_temples');
    var hasClergyPerm = hasPermission(currentUser, 'manage_clergy');
    var hasSchedulePerm = hasPermission(currentUser, 'manage_schedule');
    var hasNewsPerm = hasPermission(currentUser, 'manage_news');
    var hasAnnouncementsPerm = hasPermission(currentUser, 'manage_announcements');
    var hasSundaySchoolsPerm = hasPermission(currentUser, 'manage_sunday_schools');
    var hasAboutPerm = hasPermission(currentUser, 'manage_about');
    var hasWorshipPerm = hasPermission(currentUser, 'manage_worship');
    var hasAIPerm = hasPermission(currentUser, 'manage_ai');
    var hasOpecheniePerm = hasPermission(currentUser, 'manage_opechenie');
    var menuButtons = '';
    ...
    document.getElementById('logoutAdmin').addEventListener('click', function() { currentUser = null;
        renderAdminLogin(); });
    document.querySelectorAll('.admin-menu-btn').forEach(function(btn) {
        btn.addEventListener('click', function() { renderAdminSection(this.dataset.section); });
    });
}

// ---------- РЕНДЕРИНГ РАЗДЕЛОВ АДМИНКИ ----------
function renderAdminSection(section) {
    var content = document.getElementById('adminSectionContent');
    if (!content) return;
    ...
    switch (section) {
        case 'schedule':
            renderAdminSchedule(content);
            break;
        case 'temples':
            renderAdminTemples(content);
            break;
        case 'clergy':
            renderAdminClergy(content);
            break;
        case 'news':
            renderAdminNews(content);
            break;
        case 'announcements':
            renderAdminAnnouncements(content);
            break;
        case 'sunday-school':
            renderAdminSundaySchools(content);
            break;
        case 'about':
            renderAdminAbout(content);
            break;
        case 'worship':
            renderAdminWorship(content);
            break;
        case 'ai':
            renderAdminAI(content);
            break;
        case 'users':
            renderAdminUsers(content);
            break;
        case 'opechenie':
            renderAdminOpechenie(content);
            break;
        case 'logs':
...
// ---------- АДМИНИСТРАТИВНЫЕ ФУНКЦИИ (все, без сокращений) ----------
function renderAdminSchedule(container) {
    var html = '<h3>' + t('admin-schedule') + '</h3>' +
        ...
        '<div class="form-group"><label>' + t('temple') + '</label><select id="adminScheduleTemple" style="width:100%; padding:0.4rem;"><option value="">' + t('choose-temple') + '</option>' + data.temples.map(function(t) { return '<option value="' + t.id + '">' + escapeHtml(t.name) + '</option>'; }).join('') + '</select></div>' +
    ...
    container.addEventListener('click', function(e) {
        ...
        if (target.id === 'adminSaveScheduleBtn') {
            var templeId = parseInt(document.getElementById('adminScheduleTemple').value);
            var date = document.getElementById('adminScheduleDate').value;
            var event = document.getElementById('adminScheduleEvent').value.trim();
            if (!templeId || !date || !event) { alert('Заполните все поля'); return; }
            data.schedules.push({ id: nextId.schedule++, templeId: templeId, date: date, event: event });
            saveData();
        ...
        if (target.id === 'adminDeletePastBtn') {
            ...
            data.schedules = data.schedules.filter(function(s) { return s.date >= today; });
            saveData();
        ...
        if (target.classList.contains('admin-delete-schedule')) {
            ...
            data.schedules = data.schedules.filter(function(s) { return s.id !== id; });
            saveData();
            renderAdminSchedule(container);
        }
    });
...
function renderScheduleTable() {
    ...
    var sorted = data.schedules.slice().sort(function(a, b) { return a.date.localeCompare(b.date); });
    var table = '<table class="schedule-table"><thead><tr><th>' + t('temple') + '</th><th>' + t('date') + '</th><th>' + t('event') + '</th><th>' + t('delete') + '</th></tr></thead><tbody>';
...
function renderAdminTemples(container) {
    var html = '<h3>Управление храмами</h3>' +
    ...
    container.addEventListener('click', function(e) {
        ...
        if (target.id === 'adminTempleAddBtn') {
            ...
            ['Name', 'Summary', 'Address', 'Phone', 'Email', 'History', 'LocalHistory', 'MapCode', 'Photo'].forEach(function(f) { document.getElementById('templeForm' + f).value = ''; });
        ...
        if (target.id === 'templeFormSaveBtn') {
            var id = document.getElementById('templeFormId').value;
            var temple = {
            ...
            if (id) {
                var exist = data.temples.find(function(t) { return t.id == id; });
        ...
        if (target.classList.contains('admin-temple-delete')) {
            ...
            data.temples = data.temples.filter(function(t) { return t.id !== id; });
            saveData();
            renderAdminTemples(container);
        }
        if (target.classList.contains('admin-temple-edit')) {
            ...
            ['Name', 'Summary', 'Address', 'Phone', 'Email', 'History', 'LocalHistory', 'MapCode', 'Photo'].forEach(function(f) {
...
function renderTemplesTable() {
    ...
    var table = '<table class="schedule-table"><thead><tr><th>Название</th><th>Адрес</th><th>Действия</th></tr></thead><tbody>';
    data.temples.forEach(function(t) {
...
function renderAdminClergy(container) {
    var html = '<h3>Управление духовенством</h3>' +
    ...
    container.addEventListener('click', function(e) {
        ...
        if (target.id === 'adminClergyAddBtn') {
            ...
            ['Name', 'Rank', 'Desc', 'Photo', 'TempleIds'].forEach(function(f) { document.getElementById('clergyForm' + f).value = ''; });
        ...
        if (target.id === 'clergyFormSaveBtn') {
            var id = document.getElementById('clergyFormId').value;
            var clergy = {
                ...
                templeIds: document.getElementById('clergyFormTempleIds').value.split(',').map(function(s) { return parseInt(s.trim()); }).filter(function(n) { return !isNaN(n); })
            };
            if (!clergy.name) { alert('Имя обязательно'); return; }
            if (id) {
                var exist = data.clergy.find(function(c) { return c.id == id; });
        ...
        if (target.classList.contains('admin-clergy-delete')) {
            ...
            data.clergy = data.clergy.filter(function(c) { return c.id !== id; });
            saveData();
            renderAdminClergy(container);
        }
...
function renderClergyTable() {
    ...
    var table = '<table class="schedule-table"><thead><tr><th>Имя</th><th>Сан</th><th>Храмы</th><th>Действия</th></tr></thead><tbody>';
    data.clergy.forEach(function(c) {
        var templeNames = (c.templeIds || []).map(function(id) { return getTempleName(id); }).join(', ');
...
function renderAdminNews(container) {
    var html = '<h3>Управление новостями</h3>' +
    ...
    container.addEventListener('click', function(e) {
        ...
        if (target.id === 'newsFormSaveBtn') {
            var id = document.getElementById('newsFormId').value;
            var news = {
            ...
            if (id) {
                var exist = data.news.find(function(n) { return n.id == id; });
        ...
        if (target.classList.contains('admin-news-edit')) {
            var id = parseInt(target.dataset.id);
            var n = data.news.find(function(news) { return news.id === id; });
            if (!n) return;
...
function renderNewsTable() {
    ...
    var table = '<table class="schedule-table"><thead><tr><th>Заголовок</th><th>Дата</th><th>Действия</th></tr></thead><tbody>';
...
function renderAdminAnnouncements(container) {
    var html = '<h3>Управление объявлениями</h3>' +
    ...
    container.addEventListener('click', function(e) {
        ...
        if (target.id === 'announceFormSaveBtn') {
            ...
            if (id) {
                var exist = data.announcements.find(function(a) { return a.id == id; });
        ...
        if (target.classList.contains('admin-announce-delete')) {
            var id = parseInt(target.dataset.id);
            if (!confirm('Удалить?')) return;
            data.announcements = data.announcements.filter(function(a) { return a.id !== id; });
            saveData();
            renderAdminAnnouncements(container);
        }
        if (target.classList.contains('admin-announce-edit')) {
            var id = parseInt(target.dataset.id);
            var a = data.announcements.find(function(ann) { return ann.id === id; });
            if (!a) return;
...
function renderAnnounceTable() {
    ...
    var table = '<table class="schedule-table"><thead><tr><th>Текст</th><th>Дата</th><th>Действия</th></tr></thead><tbody>';
...
function renderAdminSundaySchools(container) {
    var html = '<h3>Управление воскресными школами</h3>' +
    ...
    container.addEventListener('click', function(e) {
        ...
        if (target.id === 'adminSSAddBtn') {
            ...
            ['Name', 'Type', 'Desc', 'Temple', 'Photo'].forEach(function(f) { document.getElementById('ssForm' + f).value = ''; });
        ...
        if (target.id === 'ssFormSaveBtn') {
            var id = document.getElementById('ssFormId').value;
            var ss = {
            ...
            if (id) {
                var exist = data.sundaySchools.find(function(s) { return s.id == id; });
        ...
        if (target.classList.contains('admin-ss-delete')) {
            ...
            data.sundaySchools = data.sundaySchools.filter(function(s) { return s.id !== id; });
            saveData();
            renderAdminSundaySchools(container);
        }
        if (target.classList.contains('admin-ss-edit')) {
            var id = parseInt(target.dataset.id);
            var s = data.sundaySchools.find(function(sch) { return sch.id === id; });
            if (!s) return;
        ...
        if (target.id === 'adminTeacherAddBtn') {
            ...
            ['Name', 'Role', 'Desc', 'Photo', 'School'].forEach(function(f) { document.getElementById('teacherForm' + f).value = ''; });
        ...
        if (target.id === 'teacherFormSaveBtn') {
            ...
            if (id) {
                var exist = data.teachers.find(function(t) { return t.id == id; });
        ...
        if (target.classList.contains('admin-teacher-delete')) {
            ...
            data.teachers = data.teachers.filter(function(t) { return t.id !== id; });
            saveData();
            renderAdminSundaySchools(container);
        }
        if (target.classList.contains('admin-teacher-edit')) {
            var id = parseInt(target.dataset.id);
            var t = data.teachers.find(function(teach) { return teach.id === id; });
            if (!t) return;
...
function renderSSTable() {
    ...
    var table = '<table class="schedule-table"><thead><tr><th>Название</th><th>Тип</th><th>Храм</th><th>Действия</th></tr></thead><tbody>';
    data.sundaySchools.forEach(function(s) {
        var templeName = getTempleName(s.templeId);
...
function renderTeacherTable() {
    ...
    var table = '<table class="schedule-table"><thead><tr><th>Имя</th><th>Роль</th><th>Школа</th><th>Действия</th></tr></thead><tbody>';
    data.teachers.forEach(function(t) {
        var school = data.sundaySchools.find(function(s) { return s.id === t.schoolId; });
        var schoolName = school ? school.name : 'Без привязки';
...
function renderAdminAbout(container) {
    var html = '<h3>Редактирование страницы "О благочинии"</h3>' +
    ...
    document.getElementById('adminAboutSaveBtn').addEventListener('click', function() {
        var newText = document.getElementById('adminAboutText').value;
...
function renderAdminWorship(container) {
    var html = '<h3>Управление богослужениями</h3>' +
    ...
    function openForm(type, item) {
        ...
        var title = type === 'prayer' ? 'Молитву' : type === 'interpret' ? 'Толкование' : 'Подготовку';
    ...
    container.addEventListener('click', function(e) {
        ...
        if (target.id === 'worshipFormSaveBtn') {
            var type = document.getElementById('worshipFormType').value;
            var id = document.getElementById('worshipFormId').value;
            var title = document.getElementById('worshipFormTitleInput').value.trim();
            var text = document.getElementById('worshipFormText').value.trim();
            ...
            if (id) {
                var exist = targetArray.find(function(i) { return i.id == id; });
        ...
        if (target.classList.contains('admin-worship-delete')) {
            ...
            data.worship[type === 'prayer' ? 'prayers' : type === 'interpret' ? 'interpretations' : 'sacraments'] = targetArray.filter(function(i) { return i.id !== id; });
            saveData();
            renderAdminWorship(container);
        }
        if (target.classList.contains('admin-worship-edit')) {
            ...
            var item = targetArray.find(function(i) { return i.id === id; });
...
function renderPrayersTable() {
    ...
    var table = '<table class="schedule-table"><thead><tr><th>Заголовок</th><th>Действия</th></tr></thead><tbody>';
...
function renderInterpretationsTable() {
    var items = data.worship.interpretations || [];
...
function renderSacramentsTable() {
    var items = data.worship.sacraments || [];
...
function renderAdminOpechenie(container) {
    var html = '<h3>Управление окормлением</h3>' +
    ...
    container.addEventListener('click', function(e) {
        ...
        if (target.id === 'adminOpechenieAddBtn') {
            ...
            ['Name', 'Responsible', 'Desc', 'Temple'].forEach(function(f) { document.getElementById('opechenieForm' + f).value = ''; });
        ...
        if (target.id === 'opechenieFormSaveBtn') {
            var id = document.getElementById('opechenieFormId').value;
            var item = {
            ...
            if (id) {
                var exist = data.opechenie.find(function(o) { return o.id == id; });
                if (exist) Object.assign(exist, item);
            } else {
        ...
        if (target.classList.contains('admin-opechenie-delete')) {
            var id = parseInt(target.dataset.id);
            if (!confirm('Удалить?')) return;
            data.opechenie = data.opechenie.filter(function(o) { return o.id !== id; });
            saveData();
            renderAdminOpechenie(container);
        }
        if (target.classList.contains('admin-opechenie-edit')) {
            var id = parseInt(target.dataset.id);
            var o = data.opechenie.find(function(op) { return op.id === id; });
            if (!o) return;
...
function renderOpechenieTable() {
    ...
    var table = '<table class="schedule-table"><thead><tr><th>Название</th><th>Ответственный</th><th>Храм</th><th>Действия</th></tr></thead><tbody>';
    data.opechenie.forEach(function(o) {
        var templeName = getTempleName(o.templeId);
...
function renderAdminUsers(container) {
    var html = '<h3>' + t('admin-users') + '</h3>' +
        ...
        '<div class="form-group"><label>Дополнительный вопрос</label><input type="text" id="userFormExtraQuestion" style="width:100%; padding:0.4rem;" placeholder="Секретный вопрос, заметка..."></div>' +
        '<div class="form-group"><label>' + t('permissions') + '</label><div id="userFormPermissions" style="display:flex; flex-wrap:wrap; gap:0.5rem;">' + allPermissions.map(function(p) { return '<label style="display:flex; align-items:center; gap:0.3rem; cursor:pointer;"><input type="checkbox" value="' + p + '" class="perm-checkbox"> ' + t(p) + '</label>'; }).join('') + '</div><small style="display:block; margin-top:0.3rem; color:#999;">Если выбрана роль, права будут автоматически заполнены. Можно изменить вручную.</small></div>' +
    ...
    document.getElementById('userFormRole').addEventListener('change', function() {
        ...
        document.querySelectorAll('.perm-checkbox').forEach(function(cb) { cb.checked = perms.includes(cb.value) || perms.includes('all'); });
    });
    ...
    container.addEventListener('click', async function(e) {
        ...
        if (target.id === 'adminAddUserBtn') {
            ...
            document.getElementById('userFormExtraQuestion').value = '';
            var perms = rolePermissions['junior'] || [];
            document.querySelectorAll('.perm-checkbox').forEach(function(cb) { cb.checked = perms.includes(cb.value) || perms.includes('all'); });
            form.style.display = 'block';
        }
        if (target.id === 'userFormCancelBtn') document.getElementById('adminUserForm').style.display = 'none';
        if (target.id === 'userFormSaveBtn') {
            var editId = document.getElementById('userFormEditId').value;
            var username = document.getElementById('userFormUsername').value.trim();
            var password = document.getElementById('userFormPassword').value.trim();
            var role = document.getElementById('userFormRole').value;
            var extraQuestion = document.getElementById('userFormExtraQuestion').value.trim();
            var checkedPerms = [];
            document.querySelectorAll('.perm-checkbox:checked').forEach(function(cb) { checkedPerms.push(cb.value); });
            if (!checkedPerms.length) { alert('Выберите хотя бы одно право'); return; }
            if (!username) { alert('Введите логин'); return; }
            if (data.users.find(function(u) { return u.username === username && u.id != editId; })) { alert('Пользователь с таким логином уже существует'); return; }
            if (editId) {
                var u = data.users.find(function(user) { return user.id == editId; });
                if (u) {
                    ...
                    u.extraQuestion = extraQuestion;
                    addLog('Редактирован пользователь ' + oldUsername + ' → ' + username, 'Роль: ' + role + ', права: ' + checkedPerms.join(', ') + ', доп. вопрос: ' + extraQuestion);
                }
            } else {
                if (!password) { alert('Введите пароль'); return; }
                var hashedPassword = await hashPassword(password);
                var newUser = { id: nextId.user++, username: username, password: hashedPassword, role: role, permissions: checkedPerms, extraQuestion: extraQuestion };
                data.users.push(newUser);
                addLog('Добавлен пользователь ' + username, 'Роль: ' + role + ', права: ' + checkedPerms.join(', ') + ', доп. вопрос: ' + extraQuestion);
            }
            saveData();
        ...
        if (target.classList.contains('admin-delete-user')) {
            var id = parseInt(target.dataset.id);
            var u = data.users.find(function(user) { return user.id === id; });
            if (!u) return;
            if (u.id === currentUser.id) { alert('Нельзя удалить себя'); return; }
            if (!confirm(t('confirm-delete'))) return;
            data.users = data.users.filter(function(user) { return user.id !== id; });
        ...
        if (target.classList.contains('admin-edit-user')) {
            ...
            document.getElementById('userFormExtraQuestion').value = u.extraQuestion || '';
...
function renderUserTable() {
    ...
    var table = '<table class="schedule-table"><thead><tr><th>' + t('username') + '</th><th>' + t('role') + '</th><th>Доп. вопрос</th><th>' + t('permissions') + '</th><th>' + t('edit') + '</th><th>' + t('delete') + '</th></tr></thead><tbody>';
    data.users.forEach(function(u) {
        var roleLabel = t('role-' + u.role) || u.role;
        var permLabels = (u.permissions || []).map(function(p) { return t(p) || p; }).join(', ');
        var isSelf = u.id === currentUser?.id;
        var extra = u.extraQuestion ? escapeHtml(u.extraQuestion.substring(0, 30)) + (u.extraQuestion.length > 30 ? '…' : '') : '';
        table += '<tr><td>' + escapeHtml(u.username) + (isSelf ? ' 👤' : '') + '</td><td>' + escapeHtml(roleLabel) + '</td><td>' + extra + '</td><td style="font-size:0.85rem;">' + escapeHtml(permLabels) + '</td><td><button class="btn btn-sm admin-edit-user" data-id="' + u.id + '">✏️</button></td><td><button class="btn btn-sm btn-danger admin-delete-user" data-id="' + u.id + '" ' + (isSelf ? 'disabled' : '') + '>🗑️</button></td></tr>';
    });
    table += '</tbody></table>';
    return table;
}
...
function renderAdminLogs(container) {
    var logs = data.logs || [];
    logs = logs.slice().sort(function(a, b) { return new Date(b.timestamp) - new Date(a.timestamp); });
    var html = '<h3>📜 Логи действий администраторов</h3>';
    if (!logs.length) {
...
function restoreVisionMode() {
    visionMode = localStorage.getItem('vision_mode') === 'on';
    updateVisionUI();
}

function updateVisionUI() {
    document.body.classList.toggle('vision', visionMode);
    var btn = document.getElementById('visionToggle');
    if (btn) btn.textContent = visionMode ? t('vision-toggle-off') : t('vision-toggle');
}
...
function initAdminTrigger() {
    var trigger = document.getElementById('secretAdminTrigger');
    if (trigger) {
...
document.addEventListener('DOMContentLoaded', function() {
    console.log('DOMContentLoaded – полная версия скрипта');
    loadData();
    initAdminTrigger();
    initVisionToggle();
    initBackToTop();
